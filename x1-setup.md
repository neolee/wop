...
下面的操作就在上一步打开的 Terminal 命令行界面下运行（具体安装命令来自 https://brew.sh ）：

* 输入：`xcode-select --install ↩︎` 这个命令会安装 Apple 开发工具包，是 Homebrew 需要的
* 输入：`/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" ↩︎` 
...
> 如果安装遇到问题，可以尝试国内的一些镜像站点，比如清华的这个 https://mirrors.tuna.tsinghua.edu.cn/help/homebrew/
